...
export class BasePage {
  ...
  async dragAndDrop(locatorFrom: string, locatorTo: string) {
    await this.page
      .locator(locatorFrom)
      .dragTo(await this.page.locator(locatorTo));
  }

  async sendKeyToElement(
    locatorType: string,
    textValue: string,
  ...
  async getElementsText(
    ...
    return await this.page
      .locator(this.getDynamicXpath(locatorType, ...dynamicValues))
      .innerText();
  }

  async selectItemInDefaultDropdown(
    locatorType: string,
    textItem: string,
    ...dynamicValues: string[]
  ) {
    const select = await this.page
      .locator(this.getDynamicXpath(locatorType, ...dynamicValues))
      .selectOption({ label: textItem });
  }

  async getSelectedItemDefaultDropdown(
    locatorType: string,
    ...dynamicValues: string[]
  ): Promise<string> {
    const dropdown = await this.page.locator(
      this.getDynamicXpath(locatorType, ...dynamicValues)
    );
  ...
  async isDropDownMultiple(
    ...
    const isMultiple = await dropdown.evaluate((select) =>
  ...
  async getCssValue(locator: string, propertyName: string): Promise<string> {
    const cssValueHandle = await this.page
      .locator(locator)
      .evaluateHandle((element, propertyName) => {
  ...
  async checkToDefaultCheckBoxOrRadio(
    locatorType: string,
    ...dynamicValues: string[]
  ) {
  ...
  async uncheckToDefaultCheckBox(
    locatorType: string,
    ...dynamicValues: string[]
  ) {
    const element = await this.page.locator(
  ...
  async getTextByJs(locatorType: string) {
    await this.page.$eval(locatorType, (element) => {
  ...
  async inputToTextboxByID(textboxID: string, value: string) {
    await this.waitForElementVisible(
      BasePageUI.DYNAMIC_TEXTBOX_BY_ID + textboxID
    );
  ...
  async clickToButtonByText(buttonText: string) {
    await this.waitForElementClickable(
      BasePageUI.DYNAMIC_BUTTON_BY_TEXT + buttonText
    );
  ...
  async clickToInputByPlaceholder(inputText: string) {
    await this.waitForElementClickable(
      BasePageUI.DYNAMIC_INPUT_BY_PLACEHOLDER + inputText
    );
    await this.clickToElement(
      BasePageUI.DYNAMIC_INPUT_BY_PLACEHOLDER + inputText
    );
  }
  ...
  async getErrorInputByPlaceholder(errorTextInput: string) {
    await this.waitForElementVisible(
      BasePageUI.DYNAMIC_ERROR_INPUT_BY_PLACEHOLDER + errorTextInput
    );
  ...
  async selectToDropdownByName(
    dropDownAttributeName: string,
    itemValue: string
  ) {
    await this.waitForElementClickable(
      BasePageUI.DYNAMIC_DROPDOWN_BY_NAME + dropDownAttributeName
    );
    await this.selectItemInDefaultDropdown(
      BasePageUI.DYNAMIC_DROPDOWN_BY_NAME + dropDownAttributeName,
      itemValue
    );
  }
  ...
  async clickToRadioButtonByLabel(radioButtonLabelName: string) {
    await this.waitForElementVisible(
      BasePageUI.DYNAMIC_RADIO_BUTTON_BY_LABEL + radioButtonLabelName
    );
    await this.checkToDefaultCheckBoxOrRadio(
      BasePageUI.DYNAMIC_RADIO_BUTTON_BY_LABEL + radioButtonLabelName
    );
  }
  ...
  async clickToCheckboxByLabel(checkboxLabelName: string) {
    await this.waitForElementVisible(
      BasePageUI.DYNAMIC_CHECKBOX_BY_LABEL + checkboxLabelName
    );
    await this.checkToDefaultCheckBoxOrRadio(
      BasePageUI.DYNAMIC_CHECKBOX_BY_LABEL + checkboxLabelName
    );
  ...
  async getTextboxValueByID(textboxID: string) {
    ...
    return await this.getElementAttribute(
      BasePageUI.DYNAMIC_TEXTBOX_BY_ID + textboxID,
      "value"
    );
  }
  ...
  async verifyAllItemsHaveStatus(
    page: Page,
    locator: string,
  ...
  async verifyAllItemsHaveCorrectType(
    page: Page,
    locator: string,
    typeToCheck: string
  ): Promise<boolean> {
  ...
  async clickMultiTimesToElement(
    page: Page,
    locator: string,
    timeToClick: number
  ) {
